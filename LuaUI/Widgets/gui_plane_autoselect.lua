...
-----------------------------
local MAX_ICONS = 10
local ICON_SIZE_X = 128
local ICON_SIZE_Y = 128
...
local function DrawBoxes(number)
	...
	Y_MIN = 0 --64 + 16  --POSITION_Y*vsy+0.5*ICON_SIZE_Y
...
local function DrawUnitModels(number)
	...
	while (ct < number) do
		...
		-- TODO: undo this awfulness
		local vehicle = UnitDefs[drawTable[ct].unitDefID].customParams.unittype == "vehicle"
		if vehicle then
			glRotate(90, 1, 0, 0)
		end
		...
		gl.PushMatrix()
		--gl.UnitMultMatrix(drawTable[ct].units)
		--gl.UnitRaw(drawTable[ct].units, true, 0)
  ...
  glDepthMask(true)
		...
		local pm = Spring.GetUnitPieceMap(unitID)
		for k, v in pairs(pm) do
			gl.Color(0, 1, 0, 0.5)
			if k == "turret" then 
				local turrPercent = (Spring.GetUnitRulesParam(unitID, "LIMB_HP_TURRET") or 100) / 100
				local color = {1 - turrPercent, turrPercent, 0, 0.5}
				if turrPercent == 0 then color = {0, 0, 0, 0.5} end
				gl.Color(color) 
			end
			gl.PushMatrix()
			gl.UnitPieceMultMatrix(unitID, v)
			gl.PolygonMode(GL.FRONT, GL.LINE)
			gl.UnitPiece(unitID, v)
			gl.PopMatrix()
