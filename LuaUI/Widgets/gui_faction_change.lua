...
local px, py = 0.4*wWidth, 0.4*wHeight
...
for _, faction in ipairs(factions) do
    if faction.techBase and faction.techBase == "CL" then
        SIDEDATA[#SIDEDATA + 1] = {
            sideName = string.lower(faction.shortName),
        }
        N_AXIS = N_AXIS + 1
    end
end
-- Parse allies forces
for _, faction in ipairs(factions) do
    if faction.techBase and faction.techBase == "IS" then
        SIDEDATA[#SIDEDATA + 1] = {
            sideName = string.lower(faction.shortName),
        }
        N_ALLIES = N_ALLIES + 1
...
for _, faction in ipairs(factions) do
    if (faction.techBase == nil) or (faction.techBase == "neutral") then
        SIDEDATA[#SIDEDATA + 1] = {
            sideName = string.lower(faction.shortName),
        }
...
function FactionChangeList()
    ...
    local sidedata = spGetSideData()
    local n = #sidedata
    local da = 2.0 * math.pi / (n - 0)  -- Random is placed at mid
    ...
    local R = RADIUS
    local r = math.pi * R / (n - 0)
    glColor(1, 1, 1, 1)
    --[[glTexture('LuaUI/Images/faction_change/' .. sidedata[1].sideName .. '.png')
    glTexRect(R - 0.5 * r, R - 0.5 * r,
              R + 0.5 * r, R + 0.5 * r)
    glTexture(false)
    if selTeam == 1 then
        glTexture('LuaUI/Images/faction_change/Selected Team.png')
        glTexRect(R - 0.5 * r, R - 0.5 * r,
                  R + 0.5 * r, R + 0.5 * r)
        glTexture(false)
    end]]
    ...
    for i=1,n do
        local ii = i - 1
        x = R + ((R - 0.7 * r) * math.sin(a0 + ii * 2.0 * math.pi / (n - 0)))
        y = R + ((R - 0.7 * r) * math.cos(a0 + ii * 2.0 * math.pi / (n - 0)))
        glTexture('LuaUI/Images/faction_change/' .. sidedata[i].sideName:lower() .. '.png')
        glTexRect(x - 0.5 * r, y - 0.5 * r,
                  x + 0.5 * r, y + 0.5 * r)
        glTexture(false)
        if selTeam == i then
            glTexture('LuaUI/Images/faction_change/Selected Team.png')
            glTexRect(x - 0.75 * r, y - 0.75 * r,
                      x + 0.75 * r, y + 0.75 * r)
            glTexture(false)
        end
    end
