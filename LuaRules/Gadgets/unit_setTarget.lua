function gadget:GetInfo()
	return {
		...
		author = "Google Frog, adapted by BrainDamage, added priority to Dgun by doo, butchered by FLOZi for MCL",
		date = "06/05/2013",
...
--export to CMD table
CMD_UNIT_SET_TARGET_NO_GROUND = GG.CustomCommands.GetCmdID("CMD_UNIT_SET_TARGET_NO_GROUND")
CMD_UNIT_SET_TARGET = GG.CustomCommands.GetCmdID("CMD_UNIT_SET_TARGET")
CMD_UNIT_CANCEL_TARGET = GG.CustomCommands.GetCmdID("CMD_UNIT_CANCEL_TARGET")
CMD_UNIT_SET_TARGET_RECTANGLE = GG.CustomCommands.GetCmdID("CMD_UNIT_SET_TARGET_RECTANGLE")
...
if gadgetHandler:IsSyncedCode() then
	...
	local spGetCommandQueue = Spring.GetUnitCommands
	...
	function gadget:UnitCmdDone(unitID, unitDefID, teamID, cmdID, cmdTag, cmdParams, cmdOptions)
		...
		if cmdID == CMD_STOP then
			...
			local activeCommandIsDgun = false--spGetCommandQueue(unitID, 0) ~= 0 and spGetCommandQueue(unitID, 1)[1].id == CMD_DGUN
	...
	function gadget:AllowCommand(unitID, unitDefID, teamID, cmdID, cmdParams, cmdOptions, cmdTag, playerID, fromSynced, fromLua)
		if Spring.GetUnitCommandCount(unitID) == 0 or not cmdOptions.meta then
