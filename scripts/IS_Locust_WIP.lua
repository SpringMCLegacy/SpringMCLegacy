...
local currHeat = 0
local maxHeat = ud.customParams.maxheat
local original_reloadTime = WeaponDefs[weapons[weaponID].weaponDef].reloadTime
local reloadTime = WeaponDefs[weapons[weaponID].weaponDef].reloadTime
weaponHeats[weaponID] = WeaponDefs[weapons[weaponID].weaponDef].customParams.heatpershot
...
function script.OverheatCheck()
	if currHeat > maxHeat then
		Overheated = true
	end
end

function script.Overheat
	if Overheated then
		Spring.SetUnitWeaponState(unitID, weaponID, {reloadTime = 2 * reloadTime})
	else
		Spring.SetUnitWeaponState(unitID, weaponID, {reloadtTime = original_reloadTime})
end
...
function script.FireWeapon(weaponID)
	currHeat = currHeat + weaponHeats[weaponID]
		if weaponID == 1 then
